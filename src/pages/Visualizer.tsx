...
import type { Metric, ModelProgress, SearchResult, VectorDoc } from '../types';
import type { EmbedResult } from '../hooks/useEmbedder';
...
export function Visualizer({ progress, embed }: VisualizerProps) {
  ...
  // Seed the sample corpus once the model is ready (re-seeds if the page remounts).
  useEffect(() => {
  ...
  const topIds = useMemo(() => new Set(results.slice(0, topK).map((r) => r.doc.id)), [results, topK]);
      ...
      <main className="layout">
        <section className="panel panel--corpus" aria-labelledby="corpus-heading">
          <h2 id="corpus-heading" className="panel__title">
            Corpus <span className="panel__hint mono">{docs.length} vectors</span>
          </h2>
